...
PROMPT.RENDER support-reply VERSION 1 VARS name "Alice" topic "billing"`,
  ...
  /* ── AI-ops ─────────────────────────────────────────────────────────── */
  {
    ...
    short: "AI-ops",
    description:
      "Operational layers that production AI systems all end up rebuilding: agent tool caches, token-stream replay, per-tenant cost budgets, moderation, lineage, SLO tracking, A/B experiments, knowledge graphs, scheduling, event logs, policy verdicts, an inference proxy, an MCP server, plus resilience & coordination primitives — distributed circuit breakers, saga-pattern workflow orchestration, and conflict-free replicated data types.",
    icon: Workflow,
    sections: [
      {
        id: "agent",
...
GRAPH.PATH alice acme                  # → one hop`,
      ...
      {
        id: "circuit",
        title: "Distributed circuit breakers",
        blurb: (
          <>
            A sliding-window failure-rate circuit breaker with the canonical
            three states: <code>closed</code> → <code>open</code> →{" "}
            <code>half_open</code>. Trips OPEN when the failure ratio over the
            recent window exceeds <code>THRESHOLD</code> (with at least{" "}
            <code>MIN</code> observations to avoid hair-trigger trips). After{" "}
            <code>COOLDOWN</code> elapses it transitions to HALFOPEN, which
            lets up to <code>HALFOPEN</code> probe calls through; consecutive
            successes return it to CLOSED, any failure re-opens. Replaces the
            per-process Hystrix / resilience4j layer every team rebuilds — one
            shared verdict across every process pointing at the same cache.
          </>
        ),
        commands: [
          { cmd: "CIRCUIT.CONFIG service [THRESHOLD f] [WINDOW n] [MIN n] [COOLDOWN ms] [HALFOPEN n]", desc: "Per-service tunables. Defaults: 50% threshold, 20-call window, 10 min samples, 30s cooldown, 3 half-open probes. Resizing the window resets outcomes." },
          { cmd: "CIRCUIT.RECORD service ok|fail", desc: "Report a downstream call outcome. May trip the breaker. Returns the post-record state." },
          { cmd: "CIRCUIT.CHECK service", desc: "Gate a downstream call. Returns {allowed, state}. Reserves a probe slot in HALFOPEN — caller must follow with RECORD." },
          { cmd: "CIRCUIT.STATE service", desc: "Full snapshot — config + counters + cooldown remaining + failure rate." },
          { cmd: "CIRCUIT.TRIP service [REASON r]", desc: "Manually open. Useful for operator-initiated drains pre-empting a flaky downstream." },
          { cmd: "CIRCUIT.RESET service", desc: "Clear back to CLOSED with empty history. Returns 1/0." },
          { cmd: "CIRCUIT.FORGET service", desc: "Drop the service entirely. Returns 1/0." },
          { cmd: "CIRCUIT.LIST", desc: "Every known service with full snapshot." },
          { cmd: "CIRCUIT.STATS", desc: "Roll-up: services / open / half_open / closed / total_requests / total_failures / total_rejected." },
        ],
        examplesLang: "bash",
        examples: `CIRCUIT.CONFIG payments THRESHOLD 0.3 WINDOW 50 MIN 20 COOLDOWN 60000 HALFOPEN 5

# Before each payment call:
CIRCUIT.CHECK payments
# → {allowed: 1, state: "closed"}

# After the call returns:
CIRCUIT.RECORD payments ok          # or "fail"

# Operator pre-emptively drains a flaky region:
CIRCUIT.TRIP payments REASON region-eu-degraded`,
      },
      {
        id: "saga",
        title: "Workflow orchestration with compensation",
        blurb: (
          <>
            The saga pattern at the cache layer. Each saga is a sequence of
            steps; each step records an optional compensating action. On
            failure, the manager returns the recorded compensations in reverse
            order (LIFO of completed steps) so the caller can run them — the
            manager stays opinion-free about how to dispatch the rollback
            (RESP command, HTTP DELETE, queue message). Replaces the
            Streams-as-orchestrator anti-pattern and the typical
            Temporal / Cadence service for the 90% case. State machine:{" "}
            <code>running → completed</code> (happy) or{" "}
            <code>running → compensating → failed</code> (rollback path).
          </>
        ),
        commands: [
          { cmd: "SAGA.START id [META k v ...]", desc: "Open a saga. Reusing a known id returns EXISTS — no silent grafting onto stale history." },
          { cmd: "SAGA.STEP id name [PAYLOAD json] [COMPENSATION cmd]", desc: "Record a completed step + its rollback. The compensation is opaque to the manager — typically a literal command string the caller will re-issue on FAIL." },
          { cmd: "SAGA.COMPLETE id", desc: "Mark the saga successful. Terminal — further STEPs are rejected." },
          { cmd: "SAGA.FAIL id [REASON r]", desc: "Transition to compensating; returns the comp list in LIFO order. Terminal." },
          { cmd: "SAGA.STATUS id", desc: "Full snapshot — state + steps + meta + timestamps + fail_reason." },
          { cmd: "SAGA.LIST [STATE running|completed|compensating|failed]", desc: "All sagas, optionally state-filtered. Sorted by id for stable output." },
          { cmd: "SAGA.FORGET id", desc: "Drop a saga from the registry. Returns 1/0." },
          { cmd: "SAGA.STATS", desc: "Per-state counts: total / running / completed / compensating / failed." },
        ],
        examplesLang: "bash",
        examples: `SAGA.START order-42 META customer alice
SAGA.STEP order-42 reserve_inventory PAYLOAD '{"sku":"A","qty":2}' COMPENSATION "RELEASE_INVENTORY A 2"
SAGA.STEP order-42 charge_card PAYLOAD '{"amount":29.99}' COMPENSATION "REFUND order-42"
SAGA.STEP order-42 ship_pkg PAYLOAD '{"id":"pkg-7"}'

# Carrier downstream returned 500 → roll back:
SAGA.FAIL order-42 REASON carrier_offline
# → ["REFUND order-42", "RELEASE_INVENTORY A 2"]
# Caller dispatches them in order.`,
      },
      {
        id: "crdt",
        ...
        blurb: (
          <>
            Four CRDT shapes with <code>MERGE</code> as the central primitive
            — joining two replicas' state without conflict regardless of
            message order or duplicates. OSS Redis ships none of these (only
            paid Redis Enterprise / CRDB does). Each key holds exactly one
            kind; mixing kinds returns <code>WRONGTYPE</code>.
            <br />
            <br />
            <strong>G-Counter</strong> — grow-only counter; per-actor slots,
            merge keeps the per-actor max.<br />
            <strong>PN-Counter</strong> — two G-Counters (P − N).<br />
            <strong>OR-Set</strong> (Observed-Remove) — each <code>SADD</code>{" "}
            mints a unique tag for the (actor, member) pair;{" "}
            <code>SREM</code> erases only the tags the remover has{" "}
            <em>seen</em>. A concurrent <code>SADD</code> on another replica
            produces a tag the remover never observed, so the element
            survives the merge.<br />
            <strong>LWW-Register</strong> — last-writer-wins keyed on (ts,
            actor) so divergent replicas converge.
          </>
        ),
        commands: [
          { cmd: "CRDT.GINCR key actor [delta]", desc: "G-Counter increment. Default delta = 1. Negative deltas rejected — grow-only is what makes the merge a max-per-actor." },
          { cmd: "CRDT.GVALUE key", desc: "Sum of every actor's slot." },
          { cmd: "CRDT.PNINCR key actor delta", desc: "PN-Counter ±. Positive deltas land in P, negative deltas land in N." },
          { cmd: "CRDT.PNVALUE key", desc: "P − N." },
          { cmd: "CRDT.SADD key actor member", desc: "OR-Set add. Mints a fresh tag so concurrent removes can't erase what they didn't observe." },
          { cmd: "CRDT.SREM key member", desc: "Erase every tag currently observed for member. Concurrent unreplicated adds survive the merge." },
          { cmd: "CRDT.SMEMBERS key", desc: "Every present element, sorted." },
          { cmd: "CRDT.SISMEMBER key member", desc: "1 if present, 0 otherwise." },
          { cmd: "CRDT.LWWSET key actor value [TS unix-ns]", desc: "LWW-Register write. Wins iff (ts, actor) is strictly greater than the stored tuple. Equal ts falls back to lex actor compare so replicas converge." },
          { cmd: "CRDT.LWWGET key", desc: "{value, ts, actor}." },
          { cmd: "CRDT.MERGE dest src", desc: "Join src's state into dest. Both keys must hold the same kind. Result lands in dest; src is untouched." },
          { cmd: "CRDT.STATE key", desc: "Full debug snapshot — per-actor slots, members, lww tuple." },
          { cmd: "CRDT.TYPE key", desc: "Kind label (g_counter / pn_counter / or_set / lww_register)." },
          { cmd: "CRDT.LIST [TYPE g_counter|pn_counter|or_set|lww_register]", desc: "Enumerate keys, optionally type-filtered. Sorted." },
          { cmd: "CRDT.FORGET key", desc: "Drop a key. Returns 1/0." },
          { cmd: "CRDT.STATS", desc: "Per-kind counts: total / g_counters / pn_counters / or_sets / lww_registers." },
        ],
        examplesLang: "bash",
        examples: `# Multi-region click counter — each region has its own actor.
CRDT.GINCR clicks region-us 1
CRDT.GINCR clicks region-eu 3
CRDT.GVALUE clicks                 # → 4

# OR-Set survives concurrent add+remove without losing the add:
CRDT.SADD set-a node-a item        # node-a sees the add
CRDT.SADD set-b node-b item        # node-b adds independently
CRDT.SREM set-a item               # node-a removes (only its observed tag)
CRDT.MERGE set-a set-b             # node-b's tag survives
CRDT.SMEMBERS set-a                # → ["item"]

# LWW register — newer ts wins regardless of arrival order:
CRDT.LWWSET flag node-a off TS 100
CRDT.LWWSET flag node-b on  TS 200
CRDT.LWWSET flag node-a off TS 150  # older — discarded
CRDT.LWWGET flag                    # → {value: "on", ts: 200, actor: "node-b"}`,
      },
    ],
  },
